...
- 정의라는건 함수의 인자로 어떤 것을 받을지, 그리고 어디까지 게산한 후 자기 자신에게 넘겨줄지를 의미합니다.
...
- 그 제한이 512MB라고 하면 프로그램이 점유하는 메모리가 최대 512MB여야 하는데, 일부 컴파일 환경에서는 스택 영역의 메모리까 별도로 1MB로 제한되어 있기도 합니다. 
...
- 그리고 채점 사이트 중에서도 설정을 따로 바꾸지 않아 스택의 메모리 제한이 문제의 메모리 제한이 별도로 작게 걸려있는 경우가 있습니다. 
</br>
...
- 두 번째 문제는 재귀 함수 관련 문제 중에서 스테디세러급인 하노이 탑 문제입니다.
...
- 이걸 알아냈다면 다 끝난건데, 원판이 1개일 때에는 자명하게 옮길 수 있고 원판이 k개일 때 옮길 수 있으면 원판이 k+1개일 때에도 옮길 수 있다는 걸 같이 봤으니 귀납적으로 이 문제를 풀 수 있음을 알게된 것입니다.
</br>
...
- func 함수를 이렇게 정의했는데 재귀가 늘 그렇듯 우리는 func(n) 내에서 func(n-1)을 불러야 하는데, 아쉽지만 지금 fun c함수에는 큰 결함이 있습니다.
